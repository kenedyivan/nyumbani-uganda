...
(function($){
    ...
    if(slider_price.length >0) {
        slider_price.slider({
            ...
            min: 50000,
            max: 5000000000,
            values: [50000, 5000000000],
            slide: function (event, ui) {
                min_price.val("Shs " + ui.values[0]);
                max_price.val("Shs " + ui.values[1]);
            }
        });
        min_price.val("Shs " + slider_price.slider("values", 0));
        max_price.val("Shs " + slider_price.slider("values", 1));
    }
    ...
    if(price_range_advanced.length >0) {
        price_range_advanced.slider({
            ...
            values: [50000, 5000000000],
            slide: function (event, ui) {
                min_price_range_hidden.val("Shs " + addCommas(ui.values[0]));
                max_price_range_hidden.val("Shs " + addCommas(ui.values[1]));

                min_price_range.text("Shs " + addCommas(ui.values[0]));
                max_price_range.text("Shs " + addCommas(ui.values[1]));
            }
        });

        min_price_range_hidden.val("Shs " + addCommas(price_range_advanced.slider("values", 0)));
        max_price_range_hidden.val("Shs " + addCommas(price_range_advanced.slider("values", 1)));

        min_price_range.text("Shs " + addCommas(price_range_advanced.slider("values", 0)));
        max_price_range.text("Shs " + addCommas(price_range_advanced.slider("values", 1)));
    }
    ...
    /* ------------------------------------------------------------------------ */
    /*  RANGE SLIDERS RATINGS
     /* ------------------------------------------------------------------------ */
    var slider_price1 = $( "#slider-price1");
    var min_price1 = $("#min-price1");
    var max_price1 = $("#max-price1");

    if(slider_price1.length >0) {
        ...
        min_price1.val(" " + slider_price1.slider("values", 0));
        max_price1.val(" " + slider_price1.slider("values", 1));
    }

    var slider_size1 = $( "#slider-size1");
    var min_size1 = $("#min-size1");
    var max_size1 = $("#max-size1");

    if(slider_size1.length >0) {
        slider_size1.slider({
            range: true,
            min: 0,
            max: 6,
            values: [1, 6],
            slide: function (event, ui) {
                min_size1.val(ui.values[0] + " sqft");
                max_size1.val(ui.values[1] + " sqft");
            }
        });
        min_size1.val(slider_size1.slider("values", 0) + " sqft");
        max_size1.val(slider_size1.slider("values", 1) + " sqft");
    }

    var price_range_advanced1 = $( ".price-range-advanced1");
    var min_price1_range1_hidden1 = $(".min-price-range-hidden1");
    var max_price1_range1_hidden1 = $(".max-price-range-hidden1");
    var min_price1_range1 = $(".min-price-range1");
    var max_price1_range1 = $(".max-price-range1");

    if(price_range_advanced1.length >0) {
        price_range_advanced1.slider({
            range: true,
            min: 1,
            max: 5,
            values: [1, 5],
            slide: function (event, ui) {
                min_price1_range1_hidden1.val(" " + addCommas(ui.values[0]));
                max_price1_range1_hidden1.val(" " + addCommas(ui.values[1]));

                min_price1_range1.text(" " + addCommas(ui.values[0]));
                max_price1_range1.text(" " + addCommas(ui.values[1]));
            }
        });

        min_price1_range1_hidden1.val("$ " + addCommas(price_range_advanced1.slider("values", 0)));
        max_price1_range1_hidden1.val("$ " + addCommas(price_range_advanced1.slider("values", 1)));

        min_price1_range1.text(" " + addCommas(price_range_advanced1.slider("values", 0)));
        max_price1_range1.text(" " + addCommas(price_range_advanced1.slider("values", 1)));
    }

    var size_range_advanced1 = $( ".size-range-advanced1");
    var min_size1_range1_hidden1 = $(".min-size-range-hidden1");
    var max_size1_range1_hidden1 = $(".max-size-range-hidden1");
    var min_size1_range1 = $(".min-size-range1");
    var max_size1_range1 = $(".max-size-range1");

    if(size_range_advanced1.length >0) {
        size_range_advanced1.slider({
            range: true,
            min: 1,
            max: 5,
            values: [1, 5],
            slide: function (event, ui) {
                min_size1_range1_hidden1.val(ui.values[0] + " sqft");
                max_size1_range1_hidden1.val(ui.values[1] + " sqft");

                min_size1_range1.text(size_range_advanced1.slider("values", 0) + " sqft");
                max_size1_range1.text(size_range_advanced1.slider("values", 1) + " sqft");
            }
        });

        min_size1_range1_hidden1.val(size_range_advanced1.slider("values", 0) + " sqft");
        max_size1_range1_hidden1.val(size_range_advanced1.slider("values", 1) + " sqft");

        min_size1_range1.text(size_range_advanced1.slider("values", 0) + " sqft");
        max_size1_range1.text(size_range_advanced1.slider("values", 1) + " sqft");
    }

    var area_range_slider1 = $( "#area-range-slider1");
    var area_range_text1 = $("#area-range-text1");
    var area_range_value1 = $("#area-range-value1");

    if(area_range_slider1.length >0) {
        area_range_slider1.slider(
            {
                value: 0,
                min: 0,
                max: 100,
                step: 1,
                slide: function (event, ui) {
                    area_range_text1.html(ui.value);
                    area_range_value1.val(ui.value);
                }
            }
        );

        area_range_text1.html(area_range_slider1.slider('value'));
        area_range_value1.val(area_range_slider1.slider('value'));
    }
